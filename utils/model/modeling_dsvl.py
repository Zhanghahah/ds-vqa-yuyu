...
def create_dsvl_model_and_transforms(
    ...
    if 'llama' or 'vqa' in args.lm_model_name_or_path.lower():
...
class DeepSpeedViLModel(nn.Module):
    def __init__(self, vis_encoder,
        ...
        self.motion_proj = self.motion_projection(self.lang_config.hidden_size//4) # 8b
        self._init_weight()
    ...
    def motion_projection(self, lang_dim):
        return nn.Sequential(
            nn.Linear(2304, lang_dim),  # an example implementation
            nn.LayerNorm(lang_dim, eps=1e-12))

    def fuse_motion_token(self, motion_q, vis_embed):
        # Key part 1: calculate context-related embedding
        # fuse motion and img_token

        ctx_embed = motion_q @ vis_embed.transpose(-1, -2)
    ...
    def forward(self, img, lang,
        ...
        if motion is not None:
            motion_feature = self.motion_proj(motion).unsqueeze(2)
            img_feature = torch.cat([img_feature, motion_feature], dim=2).mean(1)
            # img_feature = torch.cat([img_feat, mo_feat], dim=2) for (img_feat, mo_feat) in zip(img_feature, motion_feature))

        img_proj = self.projection(img_feature)

        hidden_states, attention_mask, input_labels = self.concat(img_proj, lang, attention_mask, input_labels,
                                                                  image_num)
        labels = input_labels
    ...
    def generate(self, img, lang,
        ...
        if not use_img_feat:
            ...
            img_feature = img
        if motion is not None:
            motion_feature = self.motion_proj(motion).unsqueeze(1)
            img_feature = torch.cat([img_feature, motion_feature], dim=1).mean(0).unsqueeze(0)

        img_proj = self.projection(img_feature)
        hidden_states, attention_mask, input_labels = self.concat(img_proj, lang, attention_mask, input_labels,
                                                                  image_num=[1], do_generation=True)

        generation_kwargs['output_attentions'] = True
        generation_kwargs['output_hidden_states'] = False
        ...
        return (
            output.sequences, self.tokenizer.batch_decode(output.sequences,
                                                          skip_special_tokens=True)[0], output)

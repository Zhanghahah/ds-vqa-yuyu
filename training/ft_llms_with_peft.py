...
train_df = pd.read_csv(
    "data/processed_data/hand_annotated_train_augmented_unique.csv")  # "hand_annotated_train.csv" or "hand_annotated_train_augmented_unique.csv"
...
class SaveBestModelCallback(TrainerCallback):
    ...
    def on_log(self, args, state, control, logs=None, **kwargs):
        ...
        if 'loss' in logs:
            ...
            logging.info(
                f"Epoch: {int(state.epoch)}, Step: {state.global_step}, Current training_loss: {training_loss}")
    ...
    def on_evaluate(self, args, state, control, **kwargs):
        ...
        if 'eval_loss' in state.log_history[-1]:
            ...
            logging.info(
                f"Epoch: {int(state.epoch)}, Step: {state.global_step}, Current eval_loss: {eval_loss}")  # Print current eval_loss with epoch and step
            ...
            if eval_loss < self.best_eval_loss:
                ...
                logging.info(
                    f"New best model saved at step {state.global_step} with eval_loss: {eval_loss}")  # Print loss of Best Model

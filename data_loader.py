...
class VideoDataset_NR_SlowFast_feature(data.Dataset):
    ...
    def __init__(self, data_dir, filename_path, transform, resize, database_name):
        ...
        iqa_score = pd.read_csv(filename_path)
        iqa_score.columns = ["name", "mos"]
        iqa_score["name"].apply(lambda x: str(x))
        if not iqa_score["name"][0].endswith(".mp4"):
            iqa_score["name"].apply(lambda x: x + ".mp4")
        self.video_names = iqa_score["name"].tolist()
    ...
    def __getitem__(self, idx):
        ...
        if self.database_name == 'KoNViD-1k' or self.database_name == "LIVE-Gaming":
            video_clip_min = 8
        elif self.database_name == 'youtube_ugc':
            video_clip_min = 20
